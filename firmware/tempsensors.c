...
uint8_t search_sensors(void) {
  ...
  SerialPutString( NEWLINESTR "Scanning Bus for DS18X20" NEWLINESTR );
  ow_reset();
  nSensors = 0;
  ...
  while ( diff != OW_LAST_DEVICE && nSensors < MAXSENSORS ) {
	...
	if( diff == OW_PRESENCE_ERR ) {
	  SerialPutString( "No Sensor found" NEWLINESTR );
	  break;
	}
	if( diff == OW_DATA_ERR ) {
	  SerialPutString( "Bus Error" NEWLINESTR );
	  break;
	}
...
void uart_put_temp_maxres(int32_t tval) {
  ...
  SerialPutLongInt( tval );
  SerialPutString(" °Ce-4, ");
...
void initTempSensors(void) {
  SerialPutString("Searching for OneWire temperature sensors." NEWLINESTR );
  nSensors = search_sensors();
#if DS18X20_VERBOSE
  for (i = 0; i < nSensors; i++ ) {
	SerialPutString("ID in Bus: ");
	SerialPutInt( (int)i + 1);
	SerialPutString(" - ");
  ...
  if ( DS18X20_start_meas( DS18X20_POWER_PARASITE, NULL ) 
	...
	for ( i = 0; i < nSensors; i++ ) {
	  if ( DS18X20_read_maxres( &gSensorIDs[i][0], &temp_eminus4 )
		...
		SerialPutString( "CRC Error (lost connection?)" );
		SerialPutString( NEWLINESTR );
		error++;
	  }
	}
  } else {
	SerialPutString( "Start meas. failed (short circuit?)" );
	error++;
  }

  if ( error ) {
	SerialPutString( "*** problems - rescanning bus ..." );
	nSensors = search_sensors();
	SerialPutInt( (int) nSensors );
	SerialPutString( " DS18X20 Sensor(s) available" NEWLINESTR );
	error = 0;
  }
}
